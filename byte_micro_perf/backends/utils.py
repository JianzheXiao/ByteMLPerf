...
def dump_communication_ops_report(op_name: str, dtype: str, input_shapes: List[List[int]], group_size: List[int], latency: float):
...
def dump_computation_ops_report(dtype: str, input_shapes: List[List[int]], latency: float):
    size = sum([math.prod(shape) for shape in input_shapes])    
    dtype_size = torch.finfo(getattr(torch, dtype)).bits // 8
    mb = dtype_size * size / 1024 / 1024
    algo_bw = dtype_size * size / latency / 1e3
